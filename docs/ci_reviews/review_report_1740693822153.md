...
| [中危] | `src/integrations/tavern/bridge.ts:34` | 正则处理器直接加载用户或外部的 `SettingsManager.getRegexRules()`，若无长度和复杂度限制，可能导致 ReDoS 攻击。 | 增加正则引擎复杂度校验机制和超时截断。 |
...
| [低危] | 全局状态注入处 | 各种通过 `window` 或 `EventBus` 发送的包含不可信数据（如弹窗输入、聊天内容）未经验证。 | 对任何输入边界点执行严格的类型及危险字符（如`<script>`、`javascript:`）过滤。 |
...
| [低危] | `src/integrations/tavern/bridge.ts:168` | 挂载全局节点 `globalRenderer` 时可能引发渲染异常，若未被 `ErrorBoundary` 捕获将导致整个 DOM 树损坏。 | 为外层容器追加全局级别异常拦截以防止应用级崩溃。 |
...
| [低] | `src/App.tsx:64` 事件监听 | 未对 `EventBus` 中的导航请求增加防抖 (`debounce`)，高频操作可能会导致页面频繁冲刷重绘。 | 包装防抖/节流函数，降低并发触发率。 |
...
| [低] | 混合的 CSS 与内联样式 | `bridge.ts` 中通过 `.style.` 大量修改内联样式而不是利用 Tailwind 的类名，割裂了构建系统对样式的维护。 | 将强制样式转为 `style` 标签注入或预设独立的模块化 CSS。 |

🛠️ 行动指南

高优排查: `src/integrations/tavern/bridge.ts` 中的 `new Function` 动态脚本加载以及潜在的白屏 `TypeError` 必须优先修复，可能导致依赖环境更新后的严重故障。
技术债记录: TypeScript `any` 类型泛滥和 `@ts-ignore` 过多，需在后续重构中逐步用明确的 Interface 替代；将负责所有事物的“上帝类”按单一职责进行物理拆分。