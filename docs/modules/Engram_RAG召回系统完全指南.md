...
graph TD
    ...
    subgraph Stage 2: 向量检索
        ...
        DB[(ChatDatabase)] -->|Embed| EventVec[事件向量]
...
### 4.0 双轨分层上下文 (Dual-Track Context Logic)

为增强各路检索的精度，自 V1.4 起，系统在 `Retriever` 中引入了检索词的分层处理逻辑：
- **扫描轨 (Deep Scan)**: 针对 KeywordRetrieveStep 的关键词扫描，通过 `getRecentContext(5)` 引入 5 条消息深度回溯，大幅增强对长线连续剧情中的实体捕捉能力。
- **意图轨 (Precision Track)**: 针对向量化检索 (Vector) 和重排序 (Rerank)，由于它们对历史噪音极为敏感，系统采用“先使用纯净查询词（如存在 LLM 提纯），否则仅兜底 2 条消息”的浅层增强策略。
- **冷启动保护 (Cold-Start Protection)**: (V1.4.4+) 若数据库中缺乏已向量化事件、归档事件或归档实体，Retriever 将跳过底层召回工作流并直接返回 `skippedReason`。
...
当同时启用 Embedding 和 Rerank 时，系统使用直观的求和模式计算最终相关度：

```typescript
HybridScore = BaseScore(Embedding/Keyword) + RerankScore
```
...
| **hybridAlpha** | 混合权重 (旧版本保留字段，当前调整为对等求和模式) |
...
| 版本 | 变更 |
|------|------|
| **V1.4.4**| **双轨分层上下文与计分引擎优化**: 针对 Keyword 与 Vector 引入不同深度的历史上下文回溯策略；优化混合打分算法为对等求和模式；升级自然语言检索生成提示词。 |
