...
| 模式 | 组件组合 | 特点 | 适用场景 |
|:-----|:---------|:-----|:---------|
| **Agentic (主动查阅)** | 预处理 (JSON 提取) + ID 直通 | 绕过 Embed/Rerank 等管线搜索，由判定模型根据 `{{engramIndex}}` (全量结构化 XML 记忆索引) 直接输出 JSON ID 阵列，解析出 `AgenticRecall[]` 后传入 `Retriever.agenticSearch`。底层直接提取数据，绕过一切重排环节，直接输入 `BrainRecallCache` | 对话具有强逻辑关联时，精准且低算力 |
