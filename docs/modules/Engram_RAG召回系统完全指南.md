...
| 模式 | 组件组合 | 特点 | 适用场景 |
|:-----|:---------|:-----|:---------|
| **Agentic (主动查阅)** | 预处理 (JSON 提取) + ID 直通 | 绕过 Embed/Rerank 算子海选，由 LLM 根据 `{{engramIndex}}` 目录直接钦定 ID，提取为 `AgenticRecall[]` 传入 `agenticSearch`，包装 candidates 后交由 `BrainRecallCache` | 对话具有强逻辑关联时，精准且低算力 |
