...
| 目录 | 状态 | 用途 |
|------|------|------|
| `services/database/` | ✅ 可用 | Dexie IndexedDB 数据库，ScopeManager 作用域管理 |
| `services/rag/` | ✅ **已实现** | Retriever (检索), Injector (动态注入) |
...
**全局宏注册服务** (V0.4 新增)：
- `{{engramSummaries}}` - 获取当前角色的所有 Engram 摘要内容
- `{{engramContext}}` - 动态上下文 (RAG 检索结果)
