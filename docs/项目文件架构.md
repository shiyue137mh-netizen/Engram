...
```mermaid
graph TD
    UI[L6 UI 表现层] --> State[L5 State 状态层]
    State --> Modules[L4 Modules 模块层]
    Modules --> Integrations[L3 Integrations 集成层]
    Modules --> Data[L2 Data 数据层]
    Integrations --> Data
    Data --> Config[L1 Config 配置层]
    Config --> Core[L0 Core 核心层]
```
