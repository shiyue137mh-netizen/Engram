...
│
├── stores/                # [V0.6+] Zustand 状态库 (L3 状态层)
│   ├── memoryStore.ts     # 记忆状态管理 (IndexedDB 读写，多数据库架构)
...
│   ├── DevLog/            # 开发日志
│   ├── Graph/             # 图谱可视化
│   ├── MemoryStream/      # 记忆流 (含 GraphView 图谱 Tab)
│
├── hooks/                 # React Hooks
...
| `services/preprocessing/` | ✅ **V0.8 实现** | 输入预处理系统 (Preprocessor, OutputParser) |
...
| `services/graph/` | ✅ **V0.9.4 重构** | EntityBuilder 实体提取服务，开放式 profile 结构 |

---

### `services/preprocessing/` - 输入预处理服务 (V0.8 新增)
**用户输入预处理，支持多种功能模式**

| 文件 | 导出 | 用途 |
|------|------|------|
| `Preprocessor.ts` | `preprocessor` | 预处理核心逻辑，LLM 调用和模板替换 |
| `OutputParser.ts` | `OutputParser` | 统一输出解析 (捕获 `<output>`, `<query>`, 清理 `<think>`) |
| `types.ts` | 类型定义 | `PreprocessingConfig`, `ParsedOutput` 等 |

**复用示例**：
```typescript
import { preprocessor } from '@/services/preprocessing';
const result = await preprocessor.process(userInput);
// result.output → 注入用户消息
// result.query → RAG 检索关键词
```
