...
interface EntityNode {
    ...
    layout_y?: number;
    ext?: Record<string, unknown>;  // 扩展预留
}
...
EntityBuilder.extractFromChat(chatHistory, floor)
    ↓
LLM 提取 → 消歧 → memoryStore.saveEntity()
    ↓
...
| `entity_extraction` | 实体提取 (V0.9.1) | `prompts/entity_extraction.md` |
