...
```
SummarizerService (src/modules/summarizer)
    ↓
Extractor.extract() → LLM → ExtractedEvent[]
    ↓
Pipeline.process() (src/modules/workflow)
    ↓
memoryStore.saveEvent() → IndexedDB
    ↓
MacroService.refreshCache() (src/integrations/tavern)
```
