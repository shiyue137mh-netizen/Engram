...
Engram 作为一个 React 应用挂载于 SillyTavern 之上，通过 Adapter 模式与宿主环境交互。系统核心由 **Workflow 任务编排引擎**、**IndexedDB 存储** 和 **宏注入系统** 组成。
...
graph LR
    ...
    Engram_Output -->|engramActiveEvents / engramIndex| ST_LLM[LLM 请求]
```
...
Engram 完全废弃了硬编码的检索总线与 ETL 管线，转而使用灵活的图式工作流引擎 (`WorkflowEngine`)。
...
[Service Facade] (Summarizer, EntityExtractor)
    │ 接收请求，准备上下文
    ▼
[Workflow Engine] (src/modules/workflow/core/WorkflowEngine.ts)
...
```
触发条件: Token 超限 / 蓝灯常驻事件数量超限 (activeEventCount)
    ↓
EventTrimmer.trim() (src/modules/memory)
    ↓
getEventsToMerge() → 获取待合并事件
    ↓
WorkflowEngine.run(trimmerWorkflow) → LLM 压缩
    ↓
...
- 条目内容包含 `{{engramSummaries}}` 或 Agentic RAG 新增的 `{{engramActiveEvents}}`、`{{engramIndex}}` 等宏
- 宏在 LLM 请求前被动态展开，避免由于直接修改原始聊天日志带来的污染风险

### 5.2 MacroService

```typescript
// 注册宏 in src/integrations/tavern/prompt/macros.ts
getContext().registerMacro('engramActiveEvents', () => {
    return MacroService.cachedPureActiveEvents;  // 从 IndexedDB 同步缓存的最新状态 (蓝灯活跃事件)
});
```
...
| 文件 | 职责 |
|------|------|
| `workflow/core/WorkflowEngine.ts` | 工作流执行引擎 |
