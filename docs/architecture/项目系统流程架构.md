...
- **职责**: 工作流执行引擎。
- **功能**: 按序执行 `WorkflowDefinition` 中定义的 `Steps`。管理上下文 (`JobContext`) 的生成与状态传递，支持中途取消信号 (`signal.cancelled`)，并支持基于 Step 结果 (finish, abort, jump) 的控制流跳转（注意：`jump` 未限制最大循环次数）。同时在捕获异常时提供精准的断点记录 (`currentStepName`) 以避免误报成功步数。
