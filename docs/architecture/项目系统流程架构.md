...
- **职责**: 工作流执行引擎。
- **功能**: 按序执行 `WorkflowDefinition` 中定义的 `Steps`。管理上下文 (`JobContext`) 的生成与状态传递，支持中途取消信号 (`signal.cancelled`)，并支持基于 Step 结果 (finish, abort, jump) 的控制流跳转（具备 `jump` 最大 50 次循环保护）。同时在捕获异常时提供精准的断点记录 (`currentStepName`) 以避免误报成功步数。
...
2. **Export & Encoding**: 调用 `exportChatData` 将整库数据转为 JSON 对象后，利用 `Blob` 和 `FileReader` 将数据异步转码为 Data URL 并提取 Base64，有效防止大文件阻塞主线程。
3. **Upload**: 将 Base64 字符串作为 payload，发送给宿主环境 `/api/files/upload` 接口存储为本地 JSON 备份文件。
4. **Download**: 通过 fetch GET 访问 `/user/files/` 下的备份文件，利用 `TextDecoder` 流式读取文件前部，一旦正则匹配到 `lastModified` 元数据即刻 abort 请求截断数据流，大幅节省内存与带宽，比对时间戳后决定是否全量下载覆盖本地 IndexedDB，完成被动接收。
...
过去强耦合的 `devLogStore` 现已废弃，系统底层迁移至轻量级的发布/订阅模型：
