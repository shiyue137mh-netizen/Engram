...
2. **Export & Encoding**: 调用 `exportChatData` 将整库数据转为 JSON 对象后，在主线程同步执行 `JSON.stringify` 与 `btoa(unescape(encodeURIComponent(dump)))` 进行 UTF-8 兼容的 Base64 编码，数据量大时会严重阻塞主线程。
3. **Upload**: 将 Base64 字符串作为 payload，发送给宿主环境 `/api/files/upload` 接口存储为本地 JSON 备份文件。
4. **Download**: 通过 GET 访问 `/user/files/` 下的备份文件，获取 `lastModified` 元数据比对后覆盖本地 IndexedDB，完成被动接收。
...
| `src/data/sync/SyncService.ts` | 数据同步机制 (依赖被动触发与同步编码) |
