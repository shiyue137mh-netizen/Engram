...
- 所有数据存储在独立的 IndexedDB (`ChatDatabase`) 中
...
1. **Trigger**: `EventTrimmer` 检测到 Token 超限 / 蓝灯常驻事件数量超限 (activeEventCount)。
2. **Step**: 调用 `WorkflowEngine.run(trimmerWorkflow)` 进行处理。
   - 加载 `trim.yaml`
   - 调用 LLM 压缩
   - 归档旧节点并保存新节点
...
4. **Agentic RAG 直通 (主动查阅)**:
   ...
   - 将模型赋予的 Score 映射进双轨（`embeddingScore` 与 `rerankScore`，并在 V1.4.4 中优化为对等求和模式），然后直接送入 `BrainRecallCache.process(candidates)` 执行后续类脑过滤机制。
...
| 文件 | 职责 |
|------|------|
| `src/data/db.ts` | ChatDatabase (Dexie) 多数据库架构定义 |
