...
4. **Agentic RAG 直通 (主动查阅)**:
   ...
   - 直接根据 LLM 提取的 JSON ID 数组查询 IndexedDB。
   - 将模型赋予的 Score 映射进双轨（`embeddingScore`/`rerankScore`），然后直接送入 `BrainRecallCache`，利用交集衰减天然引爆 Decay Bomb。
...
// 输出为 XML 格式，包含 <active_memory> (蓝灯) 和 <archived_summary> (绿灯) 的极致压缩字段
```
