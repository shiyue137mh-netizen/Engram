...
LLM 调用 (使用选中的预处理模板)
    ↓
WorkflowEngine.run() (ExtractTags Step 及 UserReview Step)
├── 删除 <think>...</think>
├── 捕获 <output>...</output>
├── 捕获 <query>...</query> (如有)
└── 捕获并解析 <recall_decision> (Agentic RAG)
    ↓
...
用户输入 "我们之前约定的那件事"
    ↓
Preprocessor 调用 LLM (使用 query_enhance 或 agentic_recall 模板)
    ↓
...
<output>用户提及之前的约定，需要回顾相关记忆</output>
    ↓
WorkflowEngine.run() (包含 ExtractTags / UserReview)
├── query: "约定, 承诺, 之前的对话"
