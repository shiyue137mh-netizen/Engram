...
3. **完美联动 DecayBomb**：当剧情发生大转折时，Agentic 裁判会自然抛弃上一个场景的全部激活记忆，只保留新召回的设定。此时交入缓存处理的 `candidates` 与原先的短期记忆（STM）交集为 0，这会**原汁原味地无缝触发由 `shouldTriggerDecayBomb` 控制的遗忘炸弹**，并将衰草根据 `enforceShortTermLimit` 的容量管理法则自动淘汰出局！
