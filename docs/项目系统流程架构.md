# Engram 系统流程架构 V0.9.4

> **最后更新**: 2026-01-16

## 1. 核心设计理念

### 1.1 数据双重性 (Dual-Nature Data)
数据以两种形态存在：
- **For Machine (JSON)**: 结构化的 `structured_kv`，用于过滤、索引和图谱构建
- **For Model (Text)**: `summary` 高密度文本，用于 Embedding 和 LLM 上下文

### 1.2 IndexedDB 优先 (IndexedDB-First)
- 所有数据存储在 IndexedDB (`EngramDB`)
- WorldBook 仅用于宏占位 (`{{engramSummaries}}`)
- 不再写入 WorldBook 条目

### 1.3 蓝灯/绿灯可见性 (V0.7.1)
类似酒馆世界书的触发机制：

| 状态 | 字段值 | 含义 | 可见性 |
|------|--------|------|--------|
| 🔵 **蓝灯** | `is_archived=false` | 常驻事件 | 始终显示在 `{{engramSummaries}}` |
| 🟢 **绿灯** | `is_archived=true` | 条件触发 | 仅当被 RAG 召回时临时显示 |

---

## 2. 核心组件

### 2.1 ChatManager
- **位置**: `services/database/ChatManager.ts`
- **职责**: 管理聊天状态
- **V0.6+**: 每个 chat_id 独立数据库

### 2.2 Extractor
- **位置**: `services/pipeline/Extractor.ts`
- **职责**: LLM 调用 + JSON 解析
- **输入**: 聊天消息
- **输出**: `ExtractedEvent[]` (结构化 JSON)

### 2.3 Pipeline
- **位置**: `services/pipeline/Pipeline.ts`
- **职责**: 编排 ETL 流程
- **流程**: Extractor → 保存到 IndexedDB → 刷新宏缓存

### 2.4 MacroService
- **位置**: `tavern/MacroService.ts`
- **职责**: 注册 `{{engramSummaries}}` 宏
- **V0.7.1**: 支持 `refreshCache(recalledIds?)` 传入召回 ID

### 2.5 EventTrimmer
- **位置**: `services/pipeline/EventTrimmer.ts`
- **职责**: 精简合并旧事件，同时标记 `is_archived=true`
- **流程**: 读取事件 → LLM 压缩 → 保存 Level 1 大纲 → 归档原事件

### 2.6 EntityBuilder (V0.9.4 重构)
- **位置**: `services/graph/EntityBuilder.ts`
- **职责**: 从原始对话中提取实体（角色、地点、物品等）
- **触发**: 与 SummarizerService 并行，楼层触发器
- **流程**: chatHistory → LLM 提取 → 消歧 → profileToYaml() → 保存 EntityNode
- **V0.9.4 变更**: LLM 输出开放式 `profile` 结构，取代固定字段

---

## 3. 数据流

```mermaid
graph TD
    Input[聊天消息] --> ChatMgr[ChatManager]
    ChatMgr -->|确定 chat_id| Extractor
    
    subgraph "预处理流程 (V0.8)"
        UserInput[用户输入] -->|GENERATION_AFTER_COMMANDS| Preprocessor
        Preprocessor -->|LLM 调用| LLM_Pre[LLM 预处理]
        LLM_Pre --> OutputParser[OutputParser]
        OutputParser -->|output| InjectMsg[注入用户消息]
        OutputParser -->|query| RAG[RAG 检索]
    end
    
    subgraph "写入流程"
        Extractor -->|ExtractedEvent[]| Pipeline
        Pipeline -->|EventNode| DB[(IndexedDB)]
        Pipeline --> MacroCache[MacroService.refreshCache]
    end
    
    subgraph "读取流程 (宏展开)"
        Macro[宏展开 engramSummaries] --> Store[memoryStore.getEventSummaries]
        Store -->|过滤+排序| DB
    end
    
    subgraph "精简流程"
        Trigger[Token超限] --> Trimmer[EventTrimmer]
        Trimmer -->|归档原事件| DB
        Trimmer -->|Level 1 大纲| DB
    end
    
    subgraph "RAG 召回流程"
        RAG -->|recalledIds| MacroCache
    end
    
    subgraph "实体提取流程 (V0.9.4)"
        EntityTrigger[楼层触发] -->|checkEntityExtraction| EntityBuilder
        ChatHistory[原始对话] --> EntityBuilder
        EntityBuilder -->|profile| ProfileYaml[profileToYaml]
        ProfileYaml -->|EntityNode| DB
    end
```

---

## 4. 事件聚合与排序

### 4.1 可见性过滤

```typescript
// memoryStore.getEventSummaries(recalledIds?)
const visibleEvents = events.filter(e =>
    e.level >= 1 ||                    // 大纲总是显示
    !e.is_archived ||                  // 蓝灯：未归档
    recalledSet?.has(e.id)             // 绿灯：被 RAG 召回
);
```

### 4.2 双层排序算法

```typescript
// 按消息时间线排序，确保事件按故事顺序呈现
visibleEvents.sort((a, b) => {
    // 1. 主键：消息范围起点 (故事时间线)
    const startDiff = a.source_range.start_index - b.source_range.start_index;
    if (startDiff !== 0) return startDiff;

    // 2. 次键：消息范围终点 (大纲在细节之后)
    const endDiff = a.source_range.end_index - b.source_range.end_index;
    if (endDiff !== 0) return endDiff;

    // 3. 兜底：创建时间
    return a.timestamp - b.timestamp;
});
```

---

## 5. 数据结构

### 5.1 EventNode (V0.7+)

**V0.6+ 多数据库架构**: 每个 `chat_id` 独立数据库，不再需要 `scope_id`。

```typescript
interface EventNode {
    id: string;               // UUID
    
    summary: string;          // For Model (烧录文本，用于 Embedding 和 RAG)
    structured_kv: {          // For Machine (结构化查询)
        time_anchor: string;
        role: string[];
        location: string;
        event: string;
        logic: string[];
        causality: string;
    };
    
    // 向量化
    embedding?: number[];     // 语义向量 (可选)
    is_embedded: boolean;     // 是否已向量化
    
    // 可见性控制
    is_archived: boolean;     // true=🟢绿灯(条件触发), false=🔵蓝灯(常驻)
    
    // 排序与层级
    level: number;            // 0=原始, 1+=精简大纲
    source_range: { start_index: number; end_index: number };
    significance_score: number;
    timestamp: number;
    
    // 可选
    parent_id?: string;       // 父节点 (精简来源)
}

// EntityNode - 实体节点 (V0.9.4 无边设计 + 双重结构)
interface EntityNode {
    id: string;
    name: string;
    type: EntityType;         // 'char' | 'loc' | 'item' | 'concept' | 'unknown'
    aliases: string[];        // 别名列表 (MultiEntry索引)
    description: string;      // [For Model] YAML 烧录文本
    profile: Record<string, unknown>;  // [For Machine] 开放式 KV 容器
    last_updated_at: number;
    layout_x?: number;        // 图谱布局坐标
    layout_y?: number;
}

// EntityRelation - 实体关系 (存放在 profile.relations)
interface EntityRelation {
    target: string;           // 目标实体名
    type: string;             // 关系类型
    description?: string;     // 关系细节
}

// ScopeState - 聊天状态
interface ScopeState {
    last_summarized_floor: number;
    token_usage_accumulated: number;
    last_compressed_at: number;
    active_summary_order: number;
    last_extracted_floor: number;  // V0.9.1: 上次实体提取楼层
}
```

---

## 6. 关键文件

| 文件 | 职责 |
|------|------|
| `services/database/db.ts` | EngramDB 定义 |
| `services/database/ChatManager.ts` | 聊天状态管理 |
| `services/pipeline/Pipeline.ts` | ETL 编排 |
| `services/pipeline/Extractor.ts` | LLM + JSON 解析 |
| `services/pipeline/EventTrimmer.ts` | 事件精简 + 归档 |
| `stores/memoryStore.ts` | Zustand 状态 + 可见性过滤 |
| `tavern/MacroService.ts` | 宏注册 + 缓存刷新 |
| `services/rag/Injector.ts` | RAG 注入 Hook |

