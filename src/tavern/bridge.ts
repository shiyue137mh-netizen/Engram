...
import { EventBus, EngramEvent } from '@/lib/events';
// 使用统一的 STContext 模块
import { getSTContext, getCurrentChat, getCurrentCharacter } from "@/tavern/context";
export { getSTContext, getCurrentChat, getCurrentCharacter } from "@/tavern/context";
export type { STContext, STMessage, STCharacter } from "@/tavern/context";
...
export async function initializeEngram(): Promise<void> {
    ...
    // 初始化设置管理器
    const { SettingsManager } = await import('@/services/settings/Persistence');
    ...
    try {
        const { injector } = await import('@/services/rag/Injector');
        injector.init();
    ...
    // 初始化角色删除联动服务
    try {
        const { CharacterDeleteService } = await import('@/services/CharacterDeleteService');
