...
import { entityBuilder } from '@/services/graph/EntityBuilder';
import type { EntityExtractConfig } from '@/services/api/types';
import { DEFAULT_ENTITY_CONFIG } from '@/services/api/types';
import { SwitchField } from '../APIPresets/components/FormField';
...
export const EntityConfigPanel: React.FC = () => {
    const [config, setConfig] = useState<EntityExtractConfig>({ ...DEFAULT_ENTITY_CONFIG, enabled: false });
    ...
    const loadStatus = async () => {
        try {
            ...
            setStatus(s);
            setConfig(entityBuilder.getConfig());
        } catch (e) {
