...
## <Layers className="inline-block w-4 h-4 align-text-bottom" /> 预处理与批量处理

在侧边栏的 **数据处理** → **批量处理** 页面，您可以对大规模文本进行操作。

### 历史消息批处理
如果您是中途安装的 Engram，可以使用此功能扫描之前的聊天记录，批量生成摘要和实体数据，填补记忆空白。

### <FileText className="inline-block w-4 h-4 align-text-bottom" /> 导入小说
